...
import { Package, Coffee, Shield, LayoutGrid, X, Plus, ArrowLeft, Trash2, Edit3 } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-white pb-10">
        <nav className="p-4 md:p-6 border-b flex justify-between items-center bg-gray-50 sticky top-0 z-10">
          ...
          <button onClick={() => setModalAberto(true)} className="bg-slate-900 text-white px-4 py-2 rounded-2xl font-bold flex items-center gap-2 text-sm shadow-lg active:scale-95">
            <Plus size={20}/> Novo
          </button>
        </nav>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
            {produtos.filter(p => p.local === setorAtivo).map(item => (
              <div key={item.id} className="group bg-white rounded-[24px] md:rounded-[32px] border border-gray-100 shadow-sm overflow-hidden relative transition-all hover:shadow-md">
                ...
                {/* BOTÕES: Invisíveis no PC (só aparecem no hover) e sempre visíveis no Celular */}
                <div className="absolute top-2 right-2 flex flex-col gap-2 z-20 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
                  <button 
                    ...
                    className="bg-white/95 p-3 rounded-full text-amber-500 shadow-xl active:scale-90 transition-transform"
                  >
                    <Edit3 size={18} />
                  </button>
                  <button 
                    onClick={() => { if(confirm("Excluir item?")) supabase.from('produtos').delete().eq('id', item.id).then(carregarEstoque) }} 
                    className="bg-white/95 p-3 rounded-full text-red-500 shadow-xl active:scale-90 transition-transform"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
                ...
                <div className="p-3 text-center">
                  ...
                  <div className={`inline-block px-3 py-1 rounded-full font-black text-[10px] md:text-sm ${config.bg} ${config.color}`}>
                    {item.qtd} un
                  </div>
                </div>
              </div>
